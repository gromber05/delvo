...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const upstream = await fetch(
    `${normalizedBackendUrl}/api/v1/planner/tasks/${encodeURIComponent(id)}`,
    {
      method: "GET",
      headers: {
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const upstream = await fetch(`${normalizedBackendUrl}/api/v1/planner/tasks/${encodeURIComponent(id)}`, {
    method: "PUT",
    headers: {
