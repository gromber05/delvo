...
export function ProductChat({ compact = false, className }: ProductChatProps) {
  ...
  React.useEffect(() => {
    ...
    async function loadHistory() {
      try {
        ...
        if (!listRes.ok) throw new Error("no auth")
        const listData = (await listRes.json()) as { items: ConversationItem[] }
        const latest = listData.items?.[0]
        if (!latest || cancelled) {
          if (!cancelled) setMessages([{ id: "welcome", role: "assistant", content: dictionary.chat.welcome }])
          return
        }
        const convRes = await fetch(`/api/conversations/${latest.id}`, { cache: "no-store" })
        if (!convRes.ok) throw new Error("conv not found")
        const convData = (await convRes.json()) as { item: { messages: BackendMessage[] } }
        if (cancelled) return
        const loaded: Message[] = convData.item.messages.map((m) => ({
          id: `loaded-${m.id}`,
          role: m.role,
          content: m.content,
        }))
        if (loaded.length === 0) {
          setMessages([{ id: "welcome", role: "assistant", content: dictionary.chat.welcome }])
        } else {
          setMessages(loaded)
        }
  ...
  async function send(text?: string) {
    ...
    try {
      ...
      // Keep track of the conversation ID returned by the backend
  ...
  const resetLabel = isSpanish ? "Reiniciar chat" : "Reset chat"
      ...
      <section className={cn("flex h-full min-h-0 flex-col overflow-hidden", className)}>
        ...
        <div className="shrink-0 flex items-center justify-end px-3 pt-2">
          <button
            type="button"
            onClick={resetChat}
            disabled={loading || isTyping || initializing}
            title={resetLabel}
            className="flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] text-muted-foreground hover:bg-accent hover:text-foreground transition-colors disabled:opacity-40"
          >
        ...
        <div ref={listRef} className="min-h-0 flex-1 space-y-2 overflow-y-auto overscroll-contain p-3">
          {initializing ? (
            <div className="flex items-center justify-center py-6">
              ...
              <span className="size-1.5 animate-bounce rounded-full bg-muted-foreground mx-0.5 [animation-delay:-0.1s]" />
        ...
        <form onSubmit={handleSubmit} className="shrink-0 flex items-center gap-2 border-t border-border/70 p-3">
          <input
            ...
            disabled={loading || isTyping || initializing}
          />
          <button type="submit" disabled={loading || isTyping || !input.trim() || initializing}
        ...
        <div className="mx-auto max-w-2xl">
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              disabled={loading || isTyping || initializing}
              title={resetLabel}
              className="ml-auto flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-xs font-medium text-muted-foreground hover:border-destructive/40 hover:bg-destructive/5 hover:text-destructive disabled:opacity-50 transition-colors"
            >
              <svg className="size-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99" />
              </svg>
